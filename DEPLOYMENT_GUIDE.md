...
### Common Issues
1. **Metabase connection fails:** Check credentials in config.py
2. **Missing shapefile data:** Verify src/polygons/ directory is included
3. **Performance issues:** Increase worker count in config.py
4. **Memory usage:** Monitor cache size and data loading

### Monitoring
- **Logs:** Check application logs for data loading status
- **API endpoints:** Monitor `/api/map-data` response times
- **Memory:** Watch for data loading spikes during startup

## Security Notes
- Application runs as non-root user in container
- Credentials are in config.py (handle securely)
- No external file uploads or user data storage
